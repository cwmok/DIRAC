...
def test():
    ...
    val_fixed_t1ce_list = sorted(glob.glob(f"{datapath}/BraTSReg_*/*_0000_t1ce.nii.gz"))
    val_moving_t1ce_list = sorted(glob.glob(f"{datapath}/BraTSReg_*/*_t1ce.nii.gz"))
    val_moving_t1ce_list = sorted([path for path in val_moving_t1ce_list if path not in val_fixed_t1ce_list])

    val_fixed_t2_list = sorted(glob.glob(f"{datapath}/BraTSReg_*/*_0000_t2.nii.gz"))
    ...
    valid_generator = Data.DataLoader(
        Validation_Brats_t1cet2(val_fixed_t1ce_list, val_moving_t1ce_list, val_fixed_t2_list, val_moving_t2_list, val_fixed_csv_list,
    ...
    for batch_idx, data in enumerate(valid_generator):
        ...
        with torch.no_grad():
            ...
            if output_seg:
                ...
                thresh_fw = (u_diff_fw + 0.015) * torch.ones_like(Y_ori[:, 0:1], device=Y_ori.device)
                thresh_bw = (u_diff_bw + 0.015) * torch.ones_like(X_ori[:, 0:1], device=X_ori.device)
